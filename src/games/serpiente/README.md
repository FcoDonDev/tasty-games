...
- Reglas QA: `RULES.md`. Patrón de tiempo real: [ADR 0014](../../../docs/adr/0014-realtime-timestep-interpolacion.md).
...
- Render (`components/Board.tsx`): Views nativos + Reanimated (ADR 0001).
  ...
  - **Costura del wrap (D22)**: render toroidal — nodos que cruzan la
    costura con gemela entrante (±tablero) y clip `overflow:hidden` solo en
    el contenido del tablero; midpoints torus (alias 0 o W, nunca el
    promedio de celdas opuestas). Cruce continuo, sin salto.
