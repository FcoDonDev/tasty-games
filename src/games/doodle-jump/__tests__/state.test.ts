...
function loopTick(ms: number): ReturnType<ReturnType<typeof useDoodleJumpStore.getState>['tick']> {
...
describe('store doodle-jump: publicación D8', () => {
  beforeEach(() => {
    useDoodleJumpStore.getState().reset(undefined);
    useDoodleJumpStore.setState({ paused: false });
  ...
  test('publicación throttled: 2 s de tick → ≤ 16 publicaciones (≤5 Hz)', () => {
    let publishes = 0;
    const unsub = useDoodleJumpStore.subscribe(() => {
      publishes += 1;
    });
    loopTick(2000);
    unsub();
  ...
  test('la muerte publica inmediatamente el status (overlay de fin)', () => {
    useDoodleJumpStore.getState().reset('test-lose');
    expect(useDoodleJumpStore.getState().status).toBe('playing');
    loopTick(2000);
    expect(useDoodleJumpStore.getState().status).toBe('over');
  ...
  test('pausa congela la simulación y el acumulador se descarta', () => {
    ...
    store.togglePause();
    expect(useDoodleJumpStore.getState().paused).toBe(true);
    expect(loopTick(1000)).toEqual([]);
    expect(getGame()).toBe(snapshot); // ni la referencia avanzó
    useDoodleJumpStore.getState().togglePause();
  });
  ...
  test('applyDrag y setMoveDir ignoran pausa y partida terminada', () => {
    ...
    store.applyDrag(10);
    store.setMoveDir(1);
    expect(getGame().doodler.x).toBe(before.doodler.x);
    expect(getGame().moveDir).toBe(before.moveDir);
    useDoodleJumpStore.getState().togglePause();
  });
});

describe('store doodle-jump: inputs', () => {
  beforeEach(() => {
    useDoodleJumpStore.getState().reset(undefined);
    useDoodleJumpStore.setState({ paused: false });
  });

  test('applyDrag mueve al Doodler con wrap', () => {
    const store = useDoodleJumpStore.getState();
    const before = getGame();
    store.applyDrag(50);
    expect(getGame().doodler.x).toBeCloseTo(Math.min(WORLD_W, before.doodler.x + 24), 5); // clamp 24
  });
...
describe('store doodle-jump: sentinelas E2E (§3.5)', () => {
  test('test-win: torre central asciende SOLO (score crece sin input)', () => {
    useDoodleJumpStore.getState().reset('test-win');
    const store = useDoodleJumpStore.getState();
    const before = getGame();
...
describe('store doodle-jump: stats de tick (ADR 0011)', () => {
...
describe('store doodle-jump: determinismo de run completa', () => {
  test('misma seed + misma entrada → mismo estado final (incluye drag)', () => {
    const viaStore = (): GameState => {
      useDoodleJumpStore.getState().reset('test-win');
      for (let i = 0; i < 120; i++) {
        useDoodleJumpStore.getState().applyDrag(i % 2 === 0 ? 6 : -6);
        useDoodleJumpStore.getState().tick(16);
      }
      return getGame();
    };
    const a = viaStore();
    const b = viaStore();
    expect(a.doodler).toEqual(b.doodler);
